...
React.useEffect(() => {
  ...
  const timeoutId = setTimeout(() => {
    ...
    notionLinks.forEach((link) => {
      ...
      // 2. Temporarily make the SVG invisible and add to the DOM (ensures proper bounding across all browsers)
      svg.style.position = 'absolute';
      svg.style.visibility = 'hidden';
      document.body.appendChild(svg);

      // 3. Get the bounding box (actual size) of the icon that is invisible
      const bbox = g.getBBox();

      // 4. Remove the temporary SVG from the DOM
      document.body.removeChild(svg);
      svg.style.position = '';
      svg.style.visibility = '';

      // 5. Calculate and apply the translation to center the bbox within the current viewBox
      const translateX = (currentViewBoxWidth / 2) - (bbox.x + bbox.width / 2);
      const translateY = (currentViewBoxHeight / 2) - (bbox.y + bbox.height / 2);
      g.setAttribute('transform', `translate(${translateX} ${translateY})`);

      // 6. Insert the SVG before the link's first child
